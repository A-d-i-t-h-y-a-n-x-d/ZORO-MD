...
╚════════════════════════╝`;
    ...
    async function startBot() {
        ...
        // ============================================
        // EXPRESS PAIRING ENDPOINT (FIXED)
        // ============================================
        app.post('/pair', async (req, res) => {
            ...
            try {
                ...
                while (retries < 6) {
                    try {
                        if (!tempSock.authState.creds.registered) {
                            // FIXED HERE: Converted phoneNum to string explicitly
                ...
                console.error("Pairing route error:", err.message);
                if (tempSock) {
        ...
        try {
            ...
            if (TELEGRAM_TOKEN) {
                ...
                tgBot.command('pair', async (ctx) => {
                    ...
                    try {
                        const response = await axios.post(`http://127.0.0.1:${port}/pair`, { phone: phoneNumber, userId: userId }, { timeout: 60000 });
                        ...
                        await ctx.telegram.editMessageText(ctx.chat.id, statusMsg.message_id, null, `Server Error: ${errorMsg}`);
                    }
                });
