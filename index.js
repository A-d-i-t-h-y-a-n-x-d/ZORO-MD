...
╚════════════════════════╝`;
    ...
    async function startBot() {
        ...
        // ============================================
        // EXPRESS PAIRING ENDPOINT (OPTIMIZED & FIXED)
        // ============================================
        app.post('/pair', async (req, res) => {
            ...
            try {
                ...
                if (code) {
                    ...
                    return res.status(500).json({ status: false, error: 'Failed to generate code. Please check number format.' });
                }
            } catch (err) {
                if (tempSock) {
                    try { tempSock.end(undefined); } catch {}
                }
                return res.status(500).json({ status: false, error: 'Server busy, please try again.' });
            }
        });
