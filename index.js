...
if (cluster.isPrimary || cluster.isMaster) {
    ...
    async function startBot() {
        ...
        // Auto-install Python dependencies safely and spawn Telegram Pairing Bot
        if (fs.existsSync(path.join(__dirname, 'pair_bot.py'))) {
            try {
                console.log('📦 Checking Python dependencies...');
                try {
                    execSync('pip install pyTelegramBotAPI requests python-dotenv', { stdio: 'pipe' });
                } catch {
                    try {
                        execSync('pip3 install pyTelegramBotAPI requests python-dotenv', { stdio: 'pipe' });
                    } catch {
                        execSync('python3 -m pip install pyTelegramBotAPI requests python-dotenv', { stdio: 'pipe' });
                    }
                }
                console.log('✅ Python dependencies check finished!');
            } catch (err) {
                console.log('⚠️ Could not run pip install (pip might be missing). Attempting direct run...');
            }

            // Always try starting pair_bot.py regardless of pip status
            try {
                const telegramProcess = spawn('python3', ['pair_bot.py']);
                ...
                console.error('❌ Failed to start pair_bot.py:', err.message);
            }
        }
