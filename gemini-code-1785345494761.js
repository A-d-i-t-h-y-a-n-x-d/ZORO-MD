...
bot.on('text', async (ctx) => {
    ...
    try {
        ...
        // 🛡️ macOS / Chrome Bypass Config (IP/Hosting Block വരാതിരിക്കാൻ)
        const sock = makeWASocket({
            version,
            logger: pino({ level: 'silent' }),
            printQRInTerminal: false,
            // Official macOS Desktop Client ആയി കാണിക്കുന്നു (Block തടയാൻ)
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            // Connection റെഡിയാകുന്ന മാത്രയിൽ പെയറിങ് കോഡ് ജനറേറ്റ് ചെയ്യുന്നു
            if (!state.creds.registered && !codeRequested) {
                ...
                setTimeout(async () => {
                    try {
                        ...
                        cleanupUserSession(chatId);
                        await ctx.reply(`${em.errorFormat} <b>Error generating pairing code. Please check your number or try again after 1 minute.</b>`, { parse_mode: 'HTML' });
                    }
                }, 3000);
            }
