...
╚════════════════════════╝`;
    ...
    async function startBot() {
        ...
        // ============================================
        // EXPRESS PAIRING ENDPOINT (FIXED CONNECTION LOOP)
        // ============================================
        app.post('/pair', async (req, res) => {
            ...
            try {
                ...
                // Properly close temp socket so it doesn't conflict with main bot session
                try {
        ...
        try {
            ...
            if (TELEGRAM_TOKEN) {
                ...
                tgBot.command('pair', async (ctx) => {
                    ...
                    try {
                        const response = await axios.post(`http://127.0.0.1:${port}/pair`, { phone: phoneNumber, userId: userId }, { timeout: 40000 });
                        ...
                        if (data && data.status) {
                            ...
                            // Wait 5 seconds before starting the main bot session so pairing socket closes cleanly
                            await delay(5000);
