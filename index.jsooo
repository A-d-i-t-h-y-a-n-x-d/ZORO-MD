...
if (cluster.isPrimary || cluster.isMaster) {
    ...
    async function startBot() {
        ...
        // Express Endpoint for Telegram Pairing Integration (Fixed with dynamic session folder per user)
        ...
        // Spawn Telegram Pairing Bot ONLY in worker process to prevent duplicates (Fixed to pair_bot.js)
        ...
        async function startXeonBotInc(sessionDir = './session', identifier = 'default') {
            ...
            if (process.env.SESSION_ID && identifier === 'default') {
                try {
